...
describe('UsagePage toolbar styles', () => {
  ...
  it('uses the styled top bar classes so header controls stay aligned', () => {
    ...
    expect(usagePageStyles).toMatch(/\.dailyQuotaBox\s*\{[\s\S]*?white-space:\s*nowrap;/)
    expect(usagePageStyles).toMatch(/\.dailyQuotaBox\s*\{[\s\S]*?text-overflow:\s*ellipsis;/)
    expect(usagePageStyles).toMatch(/\.dailyQuotaBox\s*\{[\s\S]*?margin-left:\s*4px;/)
    expect(usagePageStyles).toMatch(/@include mobile\s*\{[\s\S]*?\.dailyQuotaBox\s*\{[\s\S]*?margin-left:\s*0;/)
