...
describe('UsagePage toolbar styles', () => {
  ...
  it('uses the styled top bar classes so header controls stay aligned', () => {
    const tutorialIndex = usagePageSource.indexOf('className={styles.tutorialLink}')
    const dailyQuotaIndex = usagePageSource.indexOf('className={styles.dailyQuotaBox}')
    const payAsYouGoQuotaIndex = usagePageSource.indexOf('className={styles.payAsYouGoQuotaBox}')
    ...
    expect(usagePageSource).toContain('className={styles.headerInfoGroup}')
    expect(usagePageSource).toContain('className={styles.dailyQuotaBox}')
    expect(usagePageSource).toContain('className={styles.payAsYouGoQuotaBox}')
    expect(usagePageSource).toContain('role="status"')
    expect(dailyQuotaIndex).toBeGreaterThan(tutorialIndex)
    expect(payAsYouGoQuotaIndex).toBeGreaterThan(dailyQuotaIndex)
    ...
    expect(usagePageStyles).toMatch(/\.topBarCenter\s*\{[\s\S]*?justify-content:\s*flex-start;/)
    expect(usagePageStyles).toMatch(/\.headerInfoGroup\s*\{[\s\S]*?display:\s*flex;/)
    expect(usagePageStyles).toMatch(/\.headerInfoGroup\s*\{[\s\S]*?justify-content:\s*flex-start;/)
    expect(usagePageStyles).toMatch(/\.tutorialLink\s*\{[\s\S]*?white-space:\s*nowrap;/)
    expect(usagePageStyles).toMatch(/\.dailyQuotaBox,\s*\n\.payAsYouGoQuotaBox\s*\{[\s\S]*?white-space:\s*nowrap;/)
    expect(usagePageStyles).toMatch(/\.dailyQuotaBox,\s*\n\.payAsYouGoQuotaBox\s*\{[\s\S]*?text-overflow:\s*ellipsis;/)
    expect(usagePageStyles).toMatch(/\.dailyQuotaBox,\s*\n\.payAsYouGoQuotaBox\s*\{[\s\S]*?margin-left:\s*4px;/)
    expect(usagePageStyles).toMatch(/\.payAsYouGoQuotaBox\s*\{[\s\S]*?color:\s*#1d4ed8;/)
    expect(usagePageStyles).toMatch(/\.payAsYouGoQuotaBox\s*\{[\s\S]*?border:\s*1px solid color-mix\(in srgb,\s*#1d4ed8/)
    expect(usagePageStyles).toMatch(/@include mobile\s*\{[\s\S]*?\.dailyQuotaBox,\s*\n\s*\.payAsYouGoQuotaBox\s*\{[\s\S]*?margin-left:\s*0;/)
