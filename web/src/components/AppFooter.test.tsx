...
describe('AppFooter', () => {
  it('renders project links and version label', () => {
    const html = renderToStaticMarkup(<AppFooter version="v1.2.3" />);

    expect(html).toContain('© 2026');
    expect(html).toContain(`href="${GITHUB_REPOSITORY_URL}"`);
    expect(html).toContain('>llmapi_dashboard</a>');
    expect(html).toContain('License');
    expect(html).toContain('class="app-footer-line app-footer-meta"');
    expect(html).toContain('Version: v1.2.3');
    expect(html).toContain(`llmapi_dashboard</a><span>·</span><a href="${GITHUB_REPOSITORY_URL}/blob/main/LICENSE"`);
    expect(html).toContain('License</a><span>·</span><span class="app-footer-version">Version: v1.2.3</span>');
